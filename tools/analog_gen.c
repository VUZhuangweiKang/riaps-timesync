...
#define SAMPLING_INTERVAL (NS_PER_SEC / SAMPLING_RATE) // nanoseconds
...
int main(int argc, char* argv[])
{
  ...
  if (clock_gettime(CLOCK_REALTIME, &t_wait)) {
    perror("clock_gettime failed:");
    exit(-1);
  }
  while (1) {
    t_wait.tv_nsec += SAMPLING_INTERVAL;
    if (t_wait.tv_nsec >= NS_PER_SEC) {
      t_wait.tv_sec++;
      t_wait.tv_nsec -= NS_PER_SEC;
    }
    if (clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME, &t_wait, NULL)) {
